...
while 1:
    ...
    packet_str = os.read(
        socket_fd, 4096
    )  # set packet length to max packet length on the interface
    packet_count += 1
    ...
    port_src_str = packet_str[
        ETH_HLEN + ip_header_length : ETH_HLEN + ip_header_length + 2
    ]
    port_dst_str = packet_str[
        ETH_HLEN + ip_header_length + 2 : ETH_HLEN + ip_header_length + 4
    ]
    ...
    if (
        (payload_string[:3] == b"GET")
        or (payload_string[:4] == b"POST")
        or (payload_string[:4] == b"HTTP")
        or (payload_string[:3] == b"PUT")
        or (payload_string[:6] == b"DELETE")
        or (payload_string[:4] == b"HEAD")
    ):
        ...
        if current_Key in bpf_sessions:
            ...
            if binascii.hexlify(current_Key) in local_dictionary:
                ...
                if crlf in payload_string:
                    ...
                    # update dictionary
                    local_dictionary[binascii.hexlify(current_Key)] = (
                        prev_payload_string
                    )
            else:
